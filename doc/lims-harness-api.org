...
The values reported following one of the three sections above can be
retrieved by querying LIMS with the =status= command.  The call
indicates which of the three sections is being queried and the job ID
the query applies:
...
with the =result= key in the prior =ingest= command.  (Note, this
aspect to the interface is not yet final.)
